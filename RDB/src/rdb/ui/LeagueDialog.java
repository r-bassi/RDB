...
public class LeagueDialog extends javax.swing.JDialog {
  private static final Table TABLE = League.NUL;
  private final JTextField[] fields;
  private final League model;
  private Object fieldData;

  public LeagueDialog(java.awt.Frame parent, League model) {
    super(parent, true);
    initComponents();
    ...
    JTextField[] primaryFields
        = new JTextField[League.NUL.getTablePrimaryKeys().length];
    System.arraycopy(this.fields, 0, primaryFields, 0, primaryFields.length);
    if (model != null)
      for (int i = model.getDivision() == NUL_STRING
  ...
  private void populateFields() {
    ...
    this.fields[++index].setText(Transcriber.toField(model.getTvDeal()));
    this.fields[++index].setText(Transcriber.toField(model.getSponsorship()));
    this.fields[++index].setText(Transcriber.toField(model.getSalaryCap()));
    this.fields[++index].setText(Transcriber.toField(model.getCommissioner()));
    setModified(false);
  }
  ...
  private void save() {
    ...
    this.model.setTvDeal(Transcriber.toDouble(
        this.fields[++index].getText()));
    this.model.setSponsorship(Transcriber.toDouble(
        this.fields[++index].getText()));
    this.model.setSalaryCap(Transcriber.toDouble(
        this.fields[++index].getText()));
    this.model.setCommissioner(Transcriber.toString(
        this.fields[++index].getText()));
  }

  private Object getModelData(int index) {
    switch (index) {
      case 0: return model.getDivision();
      case 1: return model.getTvDeal();
      case 2: return model.getSponsorship();
      case 3: return model.getSalaryCap();
      case 4: return model.getCommissioner();
    }
