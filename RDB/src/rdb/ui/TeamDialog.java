...
public class TeamDialog extends javax.swing.JDialog {
  ...
  public TeamDialog(java.awt.Frame parent, Team model) {
    super(parent, true);
    initComponents();
    this.fields = new JTextField[] {
      this.field0, this.field1, this.field2, this.field5, this.field3,
      this.field4, this.field6, this.field7
    };
    JTextField[] primaryFields
        = new JTextField[Team.NUL.getTablePrimaryKeys().length];
    System.arraycopy(this.fields, 0, primaryFields, 0, primaryFields.length);
    if (model != null)
      for (int i = model.getCity() == NUL_STRING
  ...
  private void populateLabels() {
    JLabel[] labels = new JLabel[] {
      ...
      this.label5, this.label6, this.label7
    };
  ...
  private void populateFields() {
    int index = 0;
    this.fields[index].setText(Transcriber.toField(model.getCity()));
    this.fields[++index].setText(Transcriber.toField(model.getName()));
    this.fields[++index].setText(Transcriber.toField(model.getHomeCapacity()));
    this.fields[++index].setText(Transcriber.toField(model.getPayroll()));
    this.fields[++index].setText(Transcriber.toField(model.getMerchandise()));
    this.fields[++index].setText(Transcriber.toField(model.getTicketPrice()));
  ...
  private void save() {
    int index = 0;
    this.model.setCity(Transcriber.toString(
        this.fields[index].getText()));
    this.model.setName(Transcriber.toString(
        this.fields[++index].getText()));
    this.model.setHomeCapacity(Transcriber.toInt(
        this.fields[++index].getText()));
    this.model.setPayroll(Transcriber.toDouble(
        this.fields[++index].getText()));
    this.model.setMerchandise(Transcriber.toDouble(
        this.fields[++index].getText()));
    this.model.setTicketPrice(Transcriber.toDouble(
        this.fields[++index].getText()));
  ...
  private Object getModelData(int index) {
    switch (index) {
      case 0: return model.getCity();
      case 1: return model.getName();
      case 2: return model.getHomeCapacity();
      case 3: return model.getPayroll();
      case 4: return model.getMerchandise();
      case 5: return model.getTicketPrice();
      case 6: return model.getDivision();
      case 7: return model.getGmId();
    }
